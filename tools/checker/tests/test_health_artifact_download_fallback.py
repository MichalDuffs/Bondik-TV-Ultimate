...
def load_epg_health():
    ...
    spec = importlib.util.spec_from_file_location(
        "process_epg_health_download_fallback",
        path,
    )
...
class ArtifactDownloadFallbackTests(unittest.TestCase):

    def test_stream_skips_failed_newest_download(
        self,
    ):
        older = make_zip(
            {
    ...
    def test_epg_skips_failed_newest_download(
        self,
    ):
        older = make_zip(
            {
    ...
    def test_stream_raises_if_all_downloads_fail(
        self,
    ):
        with patch.object(
            stream_health,
            ...
            with patch.object(
                ...
                side_effect=[
                    RuntimeError(
                        "artifact 200 unavailable"
                    ),
                    RuntimeError(
                        "artifact 100 unavailable"
                    ),
                ],
            ) as request:
    ...
    def test_epg_raises_if_all_downloads_fail(
        self,
    ):
        with patch.object(
            epg_health,
